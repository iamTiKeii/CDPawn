...
export const Contracts: React.FC = () => {
  ...
  const [activePrintType, setActivePrintType] = useState<"contract" | "receipt">("contract");
  ...
  const printDocTitle = contractCodeForPrint && customerNameForPrint
    ? `${contractCodeForPrint} - ${activePrintType === "receipt" ? "Bien_Nhan" : "Hop_Dong"} - ${customerNameForPrint}`
  ...
  return (
    <div className="space-y-6 text-slate-800 animate-fade-in w-full max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 font-sans pb-10">
      ...
      <ContractForm
        ...
        onPrint={(c) => { setActivePrintType("contract"); setActivePrintContract(c); }}
        onPrintReceipt={(c) => { setActivePrintType("receipt"); setActivePrintContract(c); }}
      ...
      {activePrintContract && (() => {
        ...
        const isReceipt = activePrintType === "receipt";
        const compiledHtml = getCompiledHtml(moduleName, activePrintContract, storeDetails, {
          isNegotiated: !isReceipt && isNegotiated,
          templateCodeOverride: isReceipt && moduleName === "pawn" ? "cd_02_001.html" : undefined
        });
            ...
            <div className="modal-box bg-white border border-slate-200 text-slate-800 rounded-2xl max-w-3xl p-6 relative">
              <div className="flex justify-between items-center border-b border-slate-200 pb-3 mb-4">
                <h3 className="font-extrabold text-sm text-slate-800 flex items-center gap-2">
                  ...
                  {isReceipt ? "Xem trước biên nhận cầm đồ" : "Xem trước bản in hợp đồng"}
                </h3>
              ...
              <div className="flex justify-end gap-2 border-t border-slate-200 pt-4 mt-4">
                ...
                >
                  ...
                  {isReceipt ? "In biên nhận ngay" : "In hợp đồng ngay"}
                </button>
              </div>
            </div>
          </div>,
