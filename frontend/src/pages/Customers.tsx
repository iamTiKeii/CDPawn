...
import { Plus, Search, Edit3, X, Upload, List, AlertOctagon, CheckCircle, Users } from "lucide-react";
...
export const Customers: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      {/* 2 Primary Tabs: Khách hàng & Khách hàng bị báo xấu */}
      <div className="flex border-b border-slate-200 gap-2 bg-white px-4 pt-3 rounded-2xl shadow-sm">
        <button
          type="button"
          onClick={() => {
            setStatusFilter("");
        ...
        <button
          ...
          onClick={() => {
            ...
            setSearchParams({ status: "blacklist" });
          }}
          className={`pb-3 px-4 text-xs font-extrabold transition-all border-b-2 flex items-center gap-2 ${
            statusFilter === "blacklist"
        ...
        >
          ...
          <span>Khách hàng bị báo xấu</span>
          <span className={`badge badge-xs font-bold text-[10px] ${
            statusFilter === "blacklist" ? "bg-red-600 text-white border-none" : "bg-red-100 text-red-600 border-none"
          }`}>
            Nợ xấu
          </span>
        </button>
        ...
        <h2 className="text-lg font-bold text-slate-800 uppercase tracking-tight flex items-center gap-2">
          {statusFilter === "blacklist" ? (
            <>
              ...
              <span>Danh sách khách hàng bị báo xấu</span>
            </>
          ) : (
            <>
